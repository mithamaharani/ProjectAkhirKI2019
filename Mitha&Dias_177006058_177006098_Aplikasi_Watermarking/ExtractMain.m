...
im = rgb2ycbcr(I);
J1 = im(:,:,1); %Cantumkan saluran Y
J1=double(J1);
F1a=fft2(J1);             %%Transformasi Fourier untuk membangun sistem koordinat persegi panjang
F2a=fftshift(F1a);
ampa=abs(F2a);
[dxa,dya]=size(J1);
xa=floor(dxa/2+1);
...
fn=0.25;             %%%%Pilih posisi informasi yang disematkan sebagai busur setengah lingkaran
x0=xa;
y0=ya;
Ra=0.24*(da-1);
Ma=2*M;   
...
amp1a=zeros(dxa,dya);
for alpha=-pi:pi/2000:0           %Setengah lingkaran spektrum Fourier
...
[x2,y2]=find(amp1a==255);     %%Fourier Cartesian mengoordinasikan sistem koordinat posisi tertanam
A1a=[x2,y2];
A11a=A1a(:,1)-xa;
A12a=A1a(:,2)-ya;
Aa=[A11a,A12a];
...
amp2a=zeros(dxa,dya);        %%Lakukan transformasi ULPM
Ba=zeros(sizeAa,2);
for i=1:sizeAa
    x3=Aa(i,1);
    y3=Aa(i,2);
...
% figure;
% imshow(amp2a);      %%%Daerah dalam sistem koordinat kutub log
B1a=unique(Ba,'rows');

%%%%%%%%%%%%%%%%%%%% Melakukan sinkronisasi tanda air untuk mendapatkan matriks amplitudo sinkronisasi %%%%%%%%%%%%%%%%%%%
amp3=zeros(Ma,Na);
for i=1:sizeAa
...
rk=zeros(Ma-M+1,Na-N+1);      %%%Temukan korelasi antara templat dan tes untuk menentukan posisi tanda air
for i=1:Ma-M+1
    for j=1:Na-N+1
...
%%%%%%%%%%%%%%%%%%%%% modulasi luas dari sinkronisasi sinkronisasi %%%%%%%%%%%%%%%%%%%%%%
key=0;                  %Kunci
rng('default');
rng(key);
pa=zeros(1,Np);
...
ma=-2*zeros(1,Lc);
for i=1:Lc                    %%Korelasi dengan pa, koefisien korelasi lebih besar dari 0 adalah 1, jika tidak maka adalah 0
    amp6=amp5c(i,:);
    xg=corrcoef(amp6,pa);
    XG=xg(1,2);
    if XG>0
        ...
        ma(1,i)=0;
    end                          %%ma adalah informasi tanda air yang diekstraksi
end

error1=0;                   %%Tampilkan nomor kesalahan
for i=1:Lc
    if m(1,i)~=ma(1,i)
        error1=error1+1;
    end
